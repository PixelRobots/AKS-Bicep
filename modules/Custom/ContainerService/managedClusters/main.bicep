...
@description('Optional. Specifies the network plugin mode used for building Kubernetes network. - overlay or none.')
@allowed([
  ''
  'overlay'
])
...
@description('Optional. Specifies whether Managed Prometheus is enabled.')
param enableMonitorMetrics bool = true
...
resource managedCluster 'Microsoft.ContainerService/managedClusters@2023-03-02-preview' = {
  ...
  properties: {
    ...
    azureMonitorProfile: {
      metrics: {
        enabled: enableMonitorMetrics
        kubeStateMetrics: {
          metricAnnotationsAllowList: metricLabelsAllowlist
          metricLabelsAllowlist: metricAnnotationsAllowList
        }
      }
    }
